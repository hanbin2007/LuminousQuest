...
export { factValueAliases, normalizeComparisonText } from '../fact-value-normalization';
...
export function validateAssessmentExtraction(input: {
  ...
  for (const assessment of parsed.assessments) {
    ...
    if (
      (
        assessment.errorIds.length > 0
        || (
          assessment.facts.response === 'substantive'
          && assessment.facts.slots.length > 0
        )
      )
