...
describe('M4.1 competition runbook click path', () => {
  it('follows the documented labels through the real routes from the versioned demo start', async () => {
    ...
    expect(await screen.findByText('回到你写的路径，检查载流粒子经过的是外电路还是内电路，并说明依据。')).toBeInTheDocument();
    ...
    expect(await screen.findByText('失电子场所和得电子场所已经分开；请再检查外电路与内电路的载流粒子标注是否各自清楚。')).toBeInTheDocument();

    await user.click(screen.getByRole('link', { name: '教师视图' }));
    expect(await screen.findByRole('heading', { name: '诊断证据链' })).toBeInTheDocument();
